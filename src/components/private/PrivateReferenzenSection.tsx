...
export default function PrivateReferenzenSection() {
  useEffect(() => {
    ...
    hosts.forEach((host) => {
      ...
      function centerTo(i: number, smooth = true) {
        i = Math.max(0, Math.min(i, slides.length - 1));
        const s = slides[i];
        const vpRect = vp!.getBoundingClientRect();
        const sRect = s.getBoundingClientRect();
        // Align to LEFT edge of viewport (start), not center
      ...
      function nearestIndex() {
        ...
        slides.forEach((s, i) => {
          ...
          // Find slide whose LEFT edge is closest to viewport's LEFT edge
      ...
      const onLoad = () => setTimeout(() => { centerTo(active, false); startAuto(); }, 80);
...
@supports not (aspect-ratio: 1) {
  ...
  :is(#ref-gallery, #ref-gallery-private) .rg__media::before{ content:""; display:block; padding-top:56.25%; }
...
:is(#ref-gallery, #ref-gallery-private) .rg__nav:hover{
  background:var(--gold-1); color:#fff;
  box-shadow:0 6px 18px rgba(43,108,176,.30);
  transform:translateY(-50%) scale(1.05);
}
:is(#ref-gallery, #ref-gallery-private) .rg__nav--prev{ left:14px; }
:is(#ref-gallery, #ref-gallery-private) .rg__nav--next{ right:14px; }
...
@media (max-width: 768px) {
  ...
  :is(#ref-gallery, #ref-gallery-private) .rg__track::after{ flex: 0 0 0 !important; }
  :is(#ref-gallery, #ref-gallery-private) .rg__media{ --ratio: 4/3; --radius: 18px; box-shadow: 0 8px 22px rgba(15,37,51,.12); }
  ...
  :is(#ref-gallery, #ref-gallery-private) .rg__nav{ display: none !important; }
}
      `}} />

      <section id="ref-gallery-private" className="section reveal" aria-labelledby="ref-title-private">
        <div className="container">
          <h2 id="ref-title-private" className="section-title">Referenzen</h2>
          <p className="section-subtitle center">
            Ein kleiner Auszug realisierter Privat-PV-Anlagen – sauber geplant, montiert und dokumentiert.
          </p>

          <div className="rg" role="region" aria-roledescription="carousel" aria-label="Referenzgalerie (Privat)">
